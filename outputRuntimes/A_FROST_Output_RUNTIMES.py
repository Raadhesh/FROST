...
def _tsv_sanitize(s):
    # Keep TSV one-row-per-line safe for Excel import/paste.
    if s is None:
        return ""
    s = str(s)
    s = s.replace("\t", " ")
...
def parse_args():
    ...
    # default trials -> 10 (kept from your current script)
...
def main():
    ...
    # CHANGED: average-only block (explicitly for 10 trials) (kept from your current script)
    if trials == 10:
