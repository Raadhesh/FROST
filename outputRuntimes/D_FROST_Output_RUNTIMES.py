...
def parse_args():
    ...
    # NEW: per-trial TSV output (Excel-friendly)
...
def main():
    ...
    # TSV output header/append handling
    header_cols = [
        "trial",
        "trial_seed",
        "n",
    ...
    with open(out_path, mode, newline="") as f_out:
        ...
        for trial in range(1, trials + 1):
            ...
            # Per-trial means over this trial's signer subset S (do NOT flatten across trials)
            preprocess_pp_mean = float(statistics.mean(list(preprocess_pp.values()))) if preprocess_pp else 0.0
            sign_pp_mean = float(statistics.mean(list(sign_pp.values()))) if sign_pp else 0.0
    ...
    # --- Average-only summary explicitly for 10 trials (UNCHANGED) ---
    if trials == 10:
