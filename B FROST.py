...
class Point(object):
    ...
    def encode(self):
        ...
        Standard EdDSA-style encoding (57 bytes):
          - Encode y in little-endian (56.999 bytes),
    ...
    def decode(s):
        ...
        Decode 57-byte Ed448 point encoding:
          ...
          - recover x from curve equation:
              x^2 = (y^2 - 1) / (1 + d y^2)   mod p   (since a = -1)
        ...
        # recover x from twisted Edwards curve equation:
...
# Sanity check 1: base point must satisfy the curve equation
...
def point_neg(Pt):
    # (-x, y) in affine => (-X, Y, Z, -T) in extended
    return Point(fe_neg(Pt.X), Pt.Y, Pt.Z, fe_neg(Pt.T))


def point_add(P1, P2):
    """
    Complete addition formula for twisted Edwards curves in extended coordinates,
    valid for general parameter a (here Ed448 uses a=+1).
...
def point_double(P1):
    """
    Doubling formula for twisted Edwards curves in extended coordinates,
    valid for general parameter a (here Ed448 uses a=+1).
...
def point_mul(Pt, k):
    """
    Scalar multiplication [k]Pt (educational double-and-add).
    k is reduced mod ℓ.
    """
    k = int(k) % L_ED448
    if k == 0:
...
# Sanity check 2: subgroup order check (requires point_mul and point_eq to be defined)
...
def hash_to_scalar_nonzero_shake256(domain, outlen, *parts):
    """
    c = SHAKE256(domain || 0x00 || ctr || (len||part)... ) with digest length outlen bytes
    reduce mod ℓ, retry if c == 0
    """
    ctr = 0
    outlen = int(outlen)
    if outlen <= 0:
...
def encode_commitment_list(S, commitments):
    ...
    Encoding: for i in sorted(S): i(4B big-endian) || D_i(57B) || E_i(57B)
    """
    out = b""
    for i in sorted(S):
...
def main():
    ...
    for trial in range(1, trials + 1):
        ...
        ctx_phi = "FROST demo context (Phi)"  # also used in signing hashes
