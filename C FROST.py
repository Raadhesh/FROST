...
# Representation:
#   - Affine point: (x, y) with 0 <= x,y < p, or None for point at infinity.
# Internal scalar multiplication uses Jacobian coordinates for fewer inversions.
# =============================================================================
...
def _affine_to_jacobian(Pt):
    if Pt is None:
        # Convention: Z=0 indicates infinity
        return (1, 1, 0)
    x, y = Pt
...
def _jacobian_double(X1, Y1, Z1):
    ...
    # M = 3*(X1 - Z1^2)*(X1 + Z1^2)   (valid optimization for a=-3)
    Z1_sq = fe_mul(Z1, Z1)
...
def point_mul(Pt, k):
    # Scalar multiplication [k]Pt using double-and-add in Jacobian coordinates
    k = int(k) % N
    if k == 0 or Pt is None:
        return None

    X, Y, Z = _affine_to_jacobian(Pt)
    Qx, Qy, Qz = (1, 1, 0)  # infinity
...
def point_to_bytes(Pt):
    ...
    SEC1 uncompressed encoding:
      0x04 || x(32B big-endian) || y(32B big-endian)
    Note: point at infinity is not expected for nonzero scalars in this script.
    """
    if Pt is None:
        # Not used in commitment list (nonces are nonzero), but keep deterministic anyway.
        return b"\x00"
    x, y = Pt
...
def point_from_bytes(b):
    """
    Minimal SEC1 uncompressed decode for completeness.
    """
...
# Sanity checks (fail fast if formulas/constants are inconsistent)
...
def rand_scalar_nonzero(rng):
    # sample uniformly from Z_N^* (non-zero)
...
def lagrange_coeff_at_zero(i, S):
    # λ_i = Π_{j in S, j != i} (0 - j) / (i - j) mod N
    num = 1
    den = 1
    i = int(i) % N
    for j in S:
...
class SchnorrPoK(object):
    ...
    def __init__(self, R, mu):
        self.R = R              # P-256 affine point (x,y) or None
        self.mu = int(mu) % N   # scalar
...
def feldman_verify_share_point(C_tilde_dealer, receiver_id, share_value):
    """
    Check:
      Left:  [ f_dealer(receiver_id) ] G
      Right: Σ_{k=0..t-1} [ receiver_id^k ] Φ_{dealer,k}
    where:
      Φ_{dealer,k} = [a_{dealer,k}]G
    """
...
class Participant(object):
    ...
    def sign_share(self, rho_i, challenge_c, lambda_i):
        ...
        Produce a signature share (educational Schnorr threshold share):
          ...
          z_i = d_i + rho_i*e_i + (lambda_i * s_i * c)  mod N
        """
...
def encode_commitment_list(S, commitments):
    ...
    commitments[i] = (D_i, E_i) as points
    Encoding: for i in sorted(S): i(4B big-endian) || D_i(SEC1) || E_i(SEC1)

    Note: With nonzero nonces, D_i and E_i are never infinity, so SEC1 encodings
    are always 65 bytes here.
    """
    out = b""
    for i in sorted(S):
...
def compute_binding_factors(ctx_phi, message_bytes, S, commitments):
    ...
    Compute per-signer binding factors rho_i.
      ...
      rho_i = H_to_ZN*( DST_SIGN_RHO, i, Φ, m, encode_commitment_list(S, commitments) )
    """
...
def compute_challenge(ctx_phi, Y, R, message_bytes):
    """
    Schnorr challenge:
      c = H_to_ZN*( DST_SIGN_CHALLENGE, Φ, Y, R, m )
    """
