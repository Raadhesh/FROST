...
def point_add(P, Q):
    ...
    if Q is SECP256K1_IDENTITY:
        return P
    # Ensure affine points for consistent type
    if hasattr(P, "to_affine"):
        P = P.to_affine()
...
def point_mul_base(k):
    ...
    if k == 0:
        return SECP256K1_IDENTITY
    # Generator * scalar (we standardized _G to affine)
    return _G * k
...
def feldman_verify_share_point(C_tilde_dealer, receiver_id, share_value):
    """
    Feldman check over secp256k1:

      Left:  [ f_dealer(receiver_id) ] G
      Right: Σ_{k=0..t-1} [ receiver_id^k ] Φ_{dealer,k}

    where receiver_id^k is computed as a scalar mod N.
    """
...
class Participant(object):
    """
    KeyGen state:
      - a_{i0..i,t-1} : polynomial coefficients (scalars mod N)
      - C~_i          : commitments Φ_{ij} = [a_{ij}]G
      - σ_i           : PoK of a_{i0}
      - s_i           : long-lived secret share (scalar mod N)
      - Y_i           : public key share [s_i]G

    Signing (ephemeral) state per trial/signing:
      - d_i, e_i      : nonces (scalars mod N)
...
def encode_commitment_list(S, commitments):
    ...
    commitments[i] = (D_i_point, E_i_point)
    Encoding: for i in sorted(S): i(4B big-endian) || D_i(POINT_BYTES) || E_i(POINT_BYTES)
    """
    out = b""
    for i in sorted(S):
...
def verify_schnorr_signature(Y, ctx_phi, message_bytes, signature):
    """
    Verify a Schnorr signature (R, z) against public key Y over secp256k1:
